...
1. **Crawl** (`server/features/crawler`): starting from one article, a script opens the page, saves its title and text, and follows its links to open more pages, then follows those pages' links, and so on. It skips pages that aren't real articles, like Talk pages or Category pages. Each article gets saved as its own file in `server/documents/`.
2. **Index** (`server/features/search`): when the server starts, it reads every saved article and breaks the text into words, lowercased, punctuation removed, common words like "the" and "and" dropped. It builds a lookup that, for any word, lists every article containing it and how many times it shows up there.
